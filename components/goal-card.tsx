...
export default function GoalCard({ goal, exchangeRate }: GoalCardProps) {
  ...
  const getConvertedAmount = (amount: number, fromCurrency: "INR" | "USD") => {
    ...
    const convertedAmount = fromCurrency === "INR" ? (amount / exchangeRate) : (amount * exchangeRate)

    return formatCurrency(convertedAmount, toCurrency)
  }

  const getCurrencyIcon = (currency: "INR" | "USD") => {
    return currency === "USD" ? <DollarSign className="w-4 h-4" /> : <span className="text-sm font-bold">₹</span>
  }
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 group">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-500 bg-gray-50 px-2 py-1 rounded-full">{goal.progress}%</span>
          </div>
        </div>

        <div className="space-y-4">
          <div>
            <div className="text-2xl font-bold text-gray-900 mb-1">{formatCurrency(goal.target, goal.currency)}</div>
            <div className="text-sm text-gray-500 mb-2">≈ {getConvertedAmount(goal.target, goal.currency)}</div>
            <div className="text-sm text-gray-600">{formatCurrency(goal.saved, goal.currency)} saved</div>
          </div>
          ...
          <div className="text-sm text-gray-600">
            <span className="font-medium">{formatCurrency(goal.remaining, goal.currency)} remaining</span>
          </div>
